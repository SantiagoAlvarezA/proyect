    ...
    public void subBytes(byte[][] texto) {
        for (int i = 0; i < texto.length; i++) //Sub-Byte subroutine
        {
            for (int j = 0; j < texto.length; j++) {
                ...
                if (texto[i][j] < 0) {
                    texto[i][j] = (byte) (texto[i][j] * -1);
                }

                texto[j][i] = (byte) valorCajaS(texto[i][j] % 16);
    ...
    public void invSubBytes(byte[][] texto) {
        for (int i = 0; i < texto.length; i++) //Inverse Sub-Byte subroutine
        {
            for (int j = 0; j < texto.length; j++) {
                ...
                int hex = texto[j][i];
                texto[j][i] = (byte) invalorCajaS(texto[i][j] % 16);
            }
        }
    }
    // operacion shiftRows
...
// operacion  mixColumns
    public final void mixColumns(byte[][] texto) { //cada columna se multiplica por una matrix dada
        ...
        for (j = 0; j < 4; j++) {
            ...
            /*  texto[0][j] = (byte) (b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1]); // 2 * a0 + a3 + a2 + 3 * a1
            texto[1][j] = (byte) (b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2]); // 2 * a1 + a0 + a3 + 3 * a2
            texto[2][j] = (byte) (b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3]);
            texto[3][j] = (byte) (b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0]); */
    ...
    public final void invmixColumns(byte[][] texto) { //cada columna se multiplica por una matrix dada
        ...
        for (j = 0; j < 4; j++) {
            ...
            /* texto[0][j] = (byte) (a[0] ^ a[3] ^ a[2] ^ a[1] ); // 2 * a0 + a3 + a2 + 3 * a1
            texto[1][j] = (byte) (a[1] ^ a[0] ^ a[3] ^ a[2] ); // 2 * a1 + a0 + a3 + 3 * a2
            texto[2][j] = (byte) (a[2] ^ a[1] ^ a[0] ^ a[3] );
            texto[3][j] = (byte) (a[3] ^ a[2] ^ a[1] ^ a[0]); */
